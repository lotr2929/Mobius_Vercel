...
# deploy.env lives at project root (one level up from _dev/)
$envPath = Join-Path $PSScriptRoot "..\deploy.env"

$token     = (Get-Content $envPath | Where-Object { $_ -match '^VERCEL_TOKEN=' })      -replace '^VERCEL_TOKEN=',''
$projectId = (Get-Content $envPath | Where-Object { $_ -match '^VERCEL_PROJECT_ID=' }) -replace '^VERCEL_PROJECT_ID=',''
$teamId    = (Get-Content $envPath | Where-Object { $_ -match '^VERCEL_TEAM_ID=' })    -replace '^VERCEL_TEAM_ID=',''
