// ΓöÇΓöÇ Mobius Command Registry ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ Model persistence ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ IndexedDB storage for folder handle ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ Access management ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ Date / Time / Location ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
async function handleLocation(args, output) {
  output('≡ƒôì Detecting location...');
  try {
    const res  = await fetch('https://ipapi.co/json/', { signal: AbortSignal.timeout(4000) });
    const data = await res.json();
    if (data.error) throw new Error(data.reason || 'Unknown error');
    document.getElementById('input').value = '';
    output('≡ƒôì ' + data.city + ', ' + data.region + ', ' + data.country_name + ' (' + data.country_code + ')\n≡ƒîÉ IP: ' + data.ip + '\n≡ƒòÉ Timezone: ' + data.timezone);
  } catch (err) {
    output('Γ¥î Location unavailable: ' + err.message);
  }
}

// ΓöÇΓöÇ Google ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ

async function handleGoogle(args, output) {
  const userId = getAuth('mobius_user_id');
  if (!userId) { output('Γ¥î Not logged in.'); return; }
  output('≡ƒöì Fetching Google account info...');
  try {
    const res  = await fetch('/api/google/info?userId=' + encodeURIComponent(userId));
    const data = await res.json();
    if (data.error) { output('Γ¥î ' + data.error); return; }
    const lines = [
      '≡ƒöù Google Account',
      'Name:  ' + data.name,
      'Email: ' + data.email,
    ...
    output('Γ¥î Failed to fetch Google info: ' + err.message);
  }
}

// ΓöÇΓöÇ Device ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ

async function handleDevice(args, output) {
  const ua  = navigator.userAgent;
  const nav = navigator;
  const lines = ['≡ƒûÑ∩╕Å  Device Information'];
...
// ΓöÇΓöÇ Access ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ

async function handleAccess(output) {
  if (!('showDirectoryPicker' in window)) {
    output('Γ¥î File System Access API not supported. Use Chrome or Edge on desktop/Android.');
    return false;
  }
  try {
    rootHandle = null;
    ...
    for await (const [name, h] of handle.entries()) {
      entries.push((h.kind === 'directory' ? '≡ƒôü' : '≡ƒôä') + ' ' + name);
    }
    entries.sort();
    output('Γ£à Access granted to: ' + handle.name + '\n\n≡ƒôü Contents (' + entries.length + ' items):\n' + entries.join('\n'));
    return true;
  } catch (err) {
    if (err.name !== 'AbortError') output('Γ¥î Access denied: ' + err.message);
    else output('Γ¥î Folder selection cancelled.');
    return false;
  }
}

// ΓöÇΓöÇ Find (with Ext:, From:, To: sub-args) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
async function searchDirectory(dirHandle, query, ext, from, to, results, path) {
  for await (const [name, handle] of dirHandle.entries()) {
    ...
    results.push((handle.kind === 'directory' ? '≡ƒôü' : '≡ƒôä') + ' ' + fullPath);
...
// ΓöÇΓöÇ List ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ

async function handleList(args, output) {
  if (!await ensureAccess(output)) return;
  const entries = [];
  for await (const [name, handle] of rootHandle.entries()) {
    entries.push((handle.kind === 'directory' ? '≡ƒôü' : '≡ƒôä') + ' ' + name);
  }
  entries.sort();
  document.getElementById('input').value = '';
  output('≡ƒôü "' + rootHandle.name + '" (' + entries.length + ' items):\n' + entries.join('\n'));
}
...
// Focus: filename   ΓÇö search whole Drive, show clickable list, copy chosen file to Mobius folder
// Focus: add text   ΓÇö append text to the Mobius copy, do NOT send to AI
// Focus: update     ΓÇö write Mobius copy back to the original file
// Focus: end        ΓÇö detach file
...
window.focusSelectFile = async function(file, folderId, outputEl) {
  ...
  try {
    if (file.inMobius) {
      // Already in Mobius folder ΓÇö read directly, no copy needed
      outputEl.textContent = '≡ƒôÑ Loading "' + file.name + '"...';
      ...
      const data = await res.json();
      if (data.error) { outputEl.textContent = 'Γ¥î Read failed: ' + data.error; return; }
      ...
      // Outside Mobius ΓÇö copy in
      outputEl.textContent = '≡ƒôï Copying "' + file.name + '" to Mobius folder...';
      ...
      const data = await res.json();
      if (data.error) { outputEl.textContent = 'Γ¥î Copy failed: ' + data.error; return; }
    ...
    outputEl.innerHTML =
      '≡ƒƒó Focused on: <strong>' + focusFile.name + '</strong>' +
      (focusFile.path ? ' ΓÇö <span style="font-size:12px;color:#8d7c64;">Path: ' + focusFile.path + '</span>' : '') + '<br>' +
      (focusFile.content ? '≡ƒô¥ Current content:<br>' + md(focusFile.content) : '≡ƒôä File is empty.') + '<br><br>' +
  ...
  } catch (err) { outputEl.textContent = 'Γ¥î ' + err.message; }
};
...
async function handleFocus(args, output, outputEl) {
  ...
  if (trimmed.toLowerCase() === 'end') {
    focusFile = null;
    document.getElementById('input').value = '';
    output('≡ƒö┤ Focus ended. File detached from queries.');
    return;
  }

  // Focus: update ΓÇö write Mobius copy back to original
  if (trimmed.toLowerCase() === 'update') {
    if (!focusFile) { output('Γ¥î No file in focus.'); return; }
    if (!focusFile.originalId) { output('Γ¥î No original file to update (file was created in Mobius folder).'); return; }
    output('≡ƒöä Updating original file...');
    try {
      ...
      const data = await res.json();
      if (data.error) { output('Γ¥î Update failed: ' + data.error); return; }
      document.getElementById('input').value = '';
      output('Γ£à Original file updated successfully.');
    } catch (err) { output('Γ¥î ' + err.message); }
    return;
  }

  // Focus: add [text] ΓÇö append to Mobius copy in memory, then write full content to Drive
  // Supports: 'add some text' or 'add\nmultiline text'
  if (trimmed.toLowerCase().startsWith('add') && (trimmed.length === 3 || trimmed[3] === ' ' || trimmed[3] === '\n')) {
    if (!focusFile) { output('Γ¥î No file in focus. Use Focus: filename first.'); return; }
    const text = trimmed.slice(3).replace(/^[\s\n]+/, '');
    if (!text) { output('Usage: Focus: add [your text here]'); return; }
    output('≡ƒÆ╛ Saving to "' + focusFile.name + '"...');
    try {
      ...
      const data = await res.json();
      if (data.error) { output('Γ¥î Save failed: ' + data.error); return; }
      focusFile.content = updated;
      document.getElementById('input').value = '';
      output('Γ£à Saved to "' + focusFile.name + '".');
    } catch (err) { output('Γ¥î ' + err.message); }
    return;
  }

  // Focus: filename ΓÇö search whole Drive
  const filename = trimmed;
  if (!filename) { output('Usage: Focus: filename  |  Focus: add [text]  |  Focus: update  |  Focus: end'); return; }

  output('≡ƒöì Searching Drive for "' + filename + '"...');
  ...
  try {
    ...
    if (findData.error) { output('Γ¥î ' + findData.error); return; }

    if (findData.files.length === 0) {
      // Not found ΓÇö create new in Mobius folder
      output('≡ƒôä Not found. Creating "' + filename + '.md" in Mobius folder...');
      ...
      if (createData.error) { output('Γ¥î ' + createData.error); return; }
      ...
      if (outputEl) {
        outputEl.classList.add('html-content');
        outputEl.innerHTML = '≡ƒƒó Focused on new file: <strong>' + focusFile.name + '</strong><br>≡ƒôä File is empty.<br><br>Use "Focus: add [text]" to add content.<br>Use "Focus: end" to detach.';
      }
      return;
    }
    ...
    // Multiple matches ΓÇö show clickable list
    document.getElementById('input').value = '';
    if (outputEl) {
      outputEl.innerHTML = '≡ƒôï Found ' + findData.files.length + ' files. Tap to select:<br><br>' +
        findData.files.map((f, i) => {
          const safeId = 'focus-file-' + i;
          const label = f.inMobius ? '≡ƒôé ' : '≡ƒôä '; // folder icon = already in Mobius
          return '<div id="' + safeId + '" style="cursor:pointer;padding:6px 10px;margin-bottom:4px;background:#ede5d4;border:1px solid #c9bfae;border-radius:1px;" ' +
            ...
            label + f.name + (f.path ? ' <span style="font-size:11px;color:#8d7c64;">ΓÇö Path: ' + f.path + '</span>' : '') + '</div>';
        }).join('');
    }
  } catch (err) { output('Γ¥î ' + err.message); }
}

// ΓöÇΓöÇ Chat History ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ Command registry ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ Public API (called by index.html) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ

function detectCommand(text) {
  const trimmed = text.trim();
  const lower   = trimmed.toLowerCase();

  // Single-word shortcut ΓÇö no whitespace, no colon required
